...
const BINARY = process.env.LINUXCTL_BINARY || "linuxctl";
...
/**
 * Execute a `linuxctl` command via execFile (no shell, no injection).
 *
 * Maps (domain, action, args) onto the linuxctl CLI:
 *   linuxctl <domain> <action> [--k v ...] [--target <t>] [--format json]
 *
 * All actual work (SSH, sudo, package manager, systemd, etc.) is done
 * by the `linuxctl` Go binary. This TypeScript layer only handles MCP
